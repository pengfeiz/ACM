...
__int64 find(int x,int l,int r)
{
	if(a[x].l==l&&a[x].r==r)
		return (__int64)(p[r]-p[l])*a[x].h;
	if(l>=(a[x].l+a[x].r)/2)
		return maxx(find(2*x+1,l,r),(__int64)(p[r]-p[l])*a[x].h);
	else if(r<=(a[x].l+a[x].r)/2)
		return maxx(find(2*x,l,r),(__int64)(p[r]-p[l])*a[x].h);
}

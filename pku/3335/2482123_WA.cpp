...
{ 
	...
	{ 
		...
		if(online(l2,q))
			return 1;
		if((bintersect_a=intersect_A(l1,l2))||((bonline1=online(l1,Polygon[(i+1)%n]))&&( (!(bonline2=online(l1,Polygon[(i+2)%n])))&&((r1=multiply(Polygon[i],Polygon[(i+1)%n],l1.s)*multiply(Polygon[(i+1)%n],Polygon[(i+2)%n],l1.s))>EP)||(bonline3=online(l1,Polygon[(i+2)%n]))&&((r2=multiply(Polygon[i],Polygon[(i+2)%n],l1.s)*multiply(Polygon[(i+2)%n],Polygon[(i+3)%n],l1.s))>EP))))
			c++; 
	}      
	if(c%2==1)         
		return 0;      

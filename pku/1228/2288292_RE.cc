...
void tubao()
{
	...
	for(i=2;i<n;i++)
	{
		while( p && cha( a[d[p]].x-a[d[p-1]].x, a[d[p]].y-a[d[p-1]].y, a[i].x-a[d[p-1]].x, a[i].y-a[d[p-1]].y)>=0)
			u[d[p--]]=false;
		d[++p]=i;
		u[i]=true;
	}
	for(i=n-1;i>0;i--)
	{
		if(!u[i])
		{
			while(cha( a[d[p]].x-a[d[p-1]].x, a[d[p]].y-a[d[p-1]].y, a[i].x-a[d[p-1]].x, a[i].y-a[d[p-1]].y)>=0)
				u[d[p--]]=false;
			d[++p]=i;
			u[i]=true;
		}

...
int main()
{
	...
	for ( j = 0; j < len; j++ )
	{
	    if ( isalpha( books[i].origName[j] ) )
	    {
		books[i].name[books[i].len++] = tolower(books[i].origName[j]);
	    }
	}
    ...
    /* set the values for the first row of the table */
    for ( i = 0; i < N; i++ )
    {
    ...
    /* fill in the rest of the values in the table */
    for ( i = 1; i < 10; i++ )
    {
	for ( j = i; j < N; j++ )
	{
    ...
    /* find which books are on the shelf */
    curX = maxR;
    curY = maxC;
    numTitles = 0;
    while( curX >= 0 )

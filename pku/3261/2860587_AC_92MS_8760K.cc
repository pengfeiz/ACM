...
//Problem: 3261  User: LiZhiXu 
//Memory: 8784K  Time: 62MS 
...
void suffix_array()
{
	...
	for(k = 1; r < N; k *= 2)
	{
		...
		for(j = 0; j < k; j++ ) sa[j] = N - j - 1;
		for(i = 0; i < N; i++ ) if(SA[i] >= k) sa[j++] = SA[i] - k;
		for(i = N - 1; i >= 0; i--) SA[--C[rank[sa[i]]]] = sa[i];
		int a=-1,b=-1;
		r=-1;
		for(i=0;i<N;i++)
		{

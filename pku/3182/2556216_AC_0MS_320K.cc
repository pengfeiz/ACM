...
//	freopen("in.txt","r",stdin);
	...
	xx=999999;
	for(i=0;i<p;i++)
		if(xx>(maxx(abs(x-a[d[i]].x),abs(y-a[d[i]].y))+maxx(abs(x-a[d[i+1]].x),abs(y-a[d[i+1]].y))-maxx(abs(a[d[i]].x-a[d[i+1]].x),abs(a[d[i]].y-a[d[i+1]].y))))
			xx=maxx(abs(x-a[d[i]].x),abs(y-a[d[i]].y))+maxx(abs(x-a[d[i+1]].x),abs(y-a[d[i+1]].y))-maxx(abs(a[d[i]].x-a[d[i+1]].x),abs(a[d[i]].y-a[d[i+1]].y));
	printf("%d\n",l+xx);
	return 0;
}
